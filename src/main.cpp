...
// Include file that contains wifi credentials
#include <WifiCredentials.h>

// Network credentials defined in WifiCredentials.h
...
// Auxiliar variables to store teh current output state
...
// Define timeout time in millisecconds
const long timeoutTime = 2000;

void setup() {
  // Add to fix the serial monitor issue with 3rd party boards
    ...
    while (client.connected() && currentTime - previousTime <= timeoutTime) { // Loop while client is connected
      ...
      if (client.available()){              // if theres bytes to read from the client
        ...
        if (c == '\n'){                     // if the byte is a newline character
          ...
          // thats the end of the clients http request, so send a response
        ...
        else if (c != '\r') { // if you got anything other than a carrige return character
